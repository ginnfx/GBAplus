...
// Looks up the newest published release. Returns false when none is newer (or
// when offline). Stub for now: a networked build would query the GitHub
// releases API and fill in the version, notes, and download URL.
...
void Frontend::drawUpdateWindow() {
    ...
    if (ImGui::Button("Update now", ImVec2(130.0f, 0.0f))) {
        SDL_OpenURL(updateUrl.c_str());  // open the release page in the browser
