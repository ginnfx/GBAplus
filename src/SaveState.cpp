// All component (de)serialization lives here. These are member functions, so
// they reach each class's private state directly; collecting them in one file
// keeps the save-state format in a single place.
...
void Bus::serialize(Serializer& s) const {
    ...
    // Backup media + flash command machine.
    s.pod(backup);
    s.pod(flashState);
    s.pod(flashIdMode);
...
uint32_t Bus::romHash() const {
    // FNV-1a over the ROM header (enough to tell cartridges apart cheaply).
...
void APU::deserialize(Deserializer& d) {
    ...
    d.pod(frameAcc);
    d.pod(frameStep);
    sampleBuffer.clear();  // transient queued output is not part of the state
}
