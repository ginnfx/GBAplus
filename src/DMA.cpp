...
void DMA::onControlWrite(int channel) {
    ...
    if (ch.active) {
        return;  // already enabled; no re-latch
    }
...
void DMA::transfer(int channel) {
    ...
    const uint16_t dstAdjust = (ctrl >> 5) & 3;  // 0 inc, 1 dec, 2 fix, 3 inc+reload
    const uint16_t srcAdjust = (ctrl >> 7) & 3;  // 0 inc, 1 dec, 2 fix
    ...
    if ((ctrl & CTRL_REPEAT) && timing != TIMING_IMMEDIATE) {
        if (dstAdjust == 3) {
            ch.dst = dstStart;  // increment+reload restores the destination
        }
    } else {
        // One-shot: clear the enable bit. The write goes through the Bus,
        // whose control hook then marks the channel inactive.
