    ...
    static const auto table = [] {
        ...
        for (uint32_t idx = 0; idx < 4096; ++idx) {
            ...
            if ((top8 & 0xF0) == 0xF0) {
                ...
                handler = &ARM7TDMI::armSingleDataTransfer;
            } else if ((top8 & 0xE0) == 0x80) {
                handler = &ARM7TDMI::armBlockDataTransfer;
            } else if ((top8 & 0xC0) == 0x00) {
                if (!(top8 & 0x20) && low4 == 0x9) {
                    ...
                    if (top8 & 0x20) {
                        if (top8 & 0x02) {
                            handler = &ARM7TDMI::armPSRTransfer;
                        }
                    } else if (low4 == 0x0) {
                        handler = &ARM7TDMI::armPSRTransfer;
                    }
...
void ARM7TDMI::armDataProcessing(uint32_t opcode) {
    ...
    switch (op) {
        ...
        case 0x2: add(op1, ~op2, 1); break;
        case 0x3: add(op2, ~op1, 1); break;
        case 0x4: add(op1, op2, 0); break;
        case 0x5: add(op1, op2, c); break;
        case 0x6: add(op1, ~op2, c); break;
        case 0x7: add(op2, ~op1, c); break;
        case 0x8: result = op1 & op2; writeback = false; break;
        case 0x9: result = op1 ^ op2; writeback = false; break;
        case 0xA: add(op1, ~op2, 1); writeback = false; break;
        case 0xB: add(op1, op2, 0); writeback = false; break;
...
void ARM7TDMI::armMultiply(uint32_t opcode) {
    ...
    const bool setFlags = opcode & (1u << 20);
    const uint32_t rd = (opcode >> 16) & 0xF;
...
void ARM7TDMI::armPSRTransfer(uint32_t opcode) {
    ...
    uint32_t mask = 0;
    if (opcode & (1u << 16)) mask |= 0x000000FF;
    if (opcode & (1u << 17)) mask |= 0x0000FF00;
    if (opcode & (1u << 18)) mask |= 0x00FF0000;
    if (opcode & (1u << 19)) mask |= 0xFF000000;
...
const std::array<ARM7TDMI::ThumbHandler, 256>& ARM7TDMI::thumbTable() {
    static const auto table = [] {
        ...
        for (uint32_t i = 0; i < 256; ++i) {
            ...
            if ((i & 0xF8) == 0x18) {
                h = &ARM7TDMI::thumbAddSub;
            } else if ((i & 0xE0) == 0x00) {
                h = &ARM7TDMI::thumbShifted;
            } else if ((i & 0xE0) == 0x20) {
                h = &ARM7TDMI::thumbImmediate;
            } else if ((i & 0xFC) == 0x40) {
                h = &ARM7TDMI::thumbALU;
            } else if ((i & 0xFC) == 0x44) {
                h = &ARM7TDMI::thumbHiRegBX;
            } else if ((i & 0xF8) == 0x48) {
                h = &ARM7TDMI::thumbLoadPC;
            } else if ((i & 0xF0) == 0x50) {
                h = &ARM7TDMI::thumbLoadStoreReg;
            } else if ((i & 0xE0) == 0x60) {
                h = &ARM7TDMI::thumbLoadStoreImm;
            } else if ((i & 0xF0) == 0x80) {
                h = &ARM7TDMI::thumbLoadStoreHalf;
            } else if ((i & 0xF0) == 0x90) {
                h = &ARM7TDMI::thumbLoadStoreSP;
            } else if ((i & 0xF0) == 0xA0) {
                h = &ARM7TDMI::thumbLoadAddress;
            } else if (i == 0xB0) {
                h = &ARM7TDMI::thumbAdjustSP;
            } else if ((i & 0xF6) == 0xB4) {
                h = &ARM7TDMI::thumbPushPop;
            } else if ((i & 0xF0) == 0xC0) {
                ...
                h = &ARM7TDMI::thumbBranch;
            } else if ((i & 0xF0) == 0xF0) {
                h = &ARM7TDMI::thumbLongBranch;
            }
            t[i] = h;
        }
        return t;
...
void ARM7TDMI::thumbAddSub(uint16_t opcode) {
    ...
    const uint32_t field = (opcode >> 6) & 7;
    const uint32_t rs = (opcode >> 3) & 7;
    const uint32_t rd = opcode & 7;
...
void ARM7TDMI::thumbALU(uint16_t opcode) {
    ...
    switch (op) {
        ...
        case 0x4:
            ...
            setNZC(result, carry);
            break;
        case 0x5: regs[rd] = aluAdd(a, b, c, true); break;
        case 0x6: regs[rd] = aluAdd(a, ~b, c, true); break;
        case 0x7:
            result = shiftOperand(a, 3, b & 0xFF, false, carry);
            regs[rd] = result;
            setNZC(result, carry);
            break;
        case 0x8: setNZ(a & b); break;
        case 0x9: regs[rd] = aluAdd(0, ~b, 1, true); break;
        case 0xA: aluAdd(a, ~b, 1, true); break;
        case 0xB: aluAdd(a, b, 0, true); break;
        ...
        switch (op) {
            ...
            case 1:
                regs[rd] = static_cast<uint32_t>(
                    static_cast<int8_t>(bus.read8(addr)));
                break;
            case 2: regs[rd] = bus.read16(addr); break;
            case 3:
                regs[rd] = static_cast<uint32_t>(
                    static_cast<int16_t>(bus.read16(addr)));
                break;
