...
constexpr int WINDOW_WIDTH  = 480;  // 240x160 at 2x
constexpr int WINDOW_HEIGHT = 320;

// One frame of the 16.78 MHz clock: 228 scanlines x 1232 cycles.
...
// Until the CPU reports real instruction timings, approximate every
// instruction as a flat 4 cycles to keep the PPU clock moving.
constexpr int APPROX_CYCLES_PER_INSTR = 4;

constexpr uint32_t REG_KEYINPUT = 0x04000130;

// GBA keypad bits (active low: 0 = pressed).
...
int runEmulator(const Options& opts) {
    ...
    std::string savPath;
    if (opts.demo) {
        // No ROM needed: paint a gradient in VRAM, enable Mode 3, and park
        // the CPU on a spin loop in EWRAM.
        ...
        bus.write32(0x02000000, 0xEAFFFFFE);  // B .
        cpu.reset();
    ...
    // RGBA8888 = R in the high byte, matching the PPU's 0xRRGGBBAA pixels.
            ...
            switch (event.type) {
                ...
                case SDL_KEYUP: {
                    ...
                    if (event.type == SDL_KEYDOWN) {
                        keyState &= static_cast<uint16_t>(~bit);  // press: 0
                    } else {
                        keyState |= bit;  // release: 1
                    }
                    break;
                }
            }
    ...
    // Persist cartridge SRAM before tearing the frontend down.
